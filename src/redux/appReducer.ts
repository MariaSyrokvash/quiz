import {BirdsStateType, BirdType} from './data-birds-reducer';
import {Dispatch} from 'redux';


// export type currentIndexType = 0 | 1 | 2 | 3 | 4 | 5
...
type InitialStateType = {
	...
	disabledBtn: boolean
	currentIndex: number
	answer: BirdType | null
	currentSetBirds: Array<BirdType> | null
	birds: Array<BirdsStateType>
}
...
export const setIndexAC = (currentIndex: number) => {
	return {
		type: 'SET-INDEX-TO-CURRENT-LEVEL',
		currentIndex,
	} as const
}

export const setActivePageAC = (page: string | undefined) => {
	return {
...
export const isTouchedAC = (value: boolean) => {
	return {
		type: 'IS-TOUCHED',
		isTouched: value
	} as const
...
export const isColoredAC = (value: boolean) => {
	return {
		type: 'IS-COLORED',
		isColored: value
	} as const
...
export const setFinishAC = (finish: boolean) => {
	return {
		type: 'IS-CHECKED-ANSWER',
		isFinish: finish
	} as const
}
export const isNextLevelAC = (nextLevel: boolean) => {
	return {
		type: 'IS-NEXT-LEVEL',
...
export const setBirdsToCurrentLevelAC = (currentIndex: number, birds: Array<BirdsStateType>, currentSetBirds: Array<BirdType>) => {
	return {
		type: 'SET-BIRDS-TO-CURRENT-LEVEL',
		currentIndex,
		birds,
		currentSetBirds
	} as const
...
export const setRandomBirdAC = (value: BirdType) => {
	return {
		type: 'SET-RANDOM-BIRD',
		answer: value
	} as const
}
...
export const disabledBtn = (value: boolean) => {
	return {
		type: 'SET-DISABLED-ANSWER',
		disabledBtn: value
	} as const
}
...
export const appReducer = (state: InitialStateType = initialState, action: ActionsType): InitialStateType => {
	switch (action.type) {
		case 'SET-INDEX-TO-CURRENT-LEVEL': {
			return {
				...state,
				currentIndex: action.currentIndex
			}
		}
		case 'SET-BIRDS-TO-CURRENT-LEVEL': {
			return {
				...state,
				currentSetBirds: action.birds[action.currentIndex]
			}
		}
		case 'SET-RANDOM-BIRD': {
			return {
				...state,
		...
		case 'IS-NEXT-LEVEL': {
			return {
				...state,
				isNextLevel: action.isNextLevel
			}
		}
		case 'IS-COLORED': {
			return {
				...state,
				isColored: action.isColored
			}
		}
		default:
			return state
...
type ActionsType =
	ReturnType<typeof setIndexAC>
	| ReturnType<typeof setBirdsToCurrentLevelAC>
	| ReturnType<typeof setRandomBirdAC>
	| ReturnType<typeof setCorrectAnswer>
	| ReturnType<typeof disabledBtn>
	| ReturnType<typeof incrementIndex>
	| ReturnType<typeof setCurrentID>
	| ReturnType<typeof isTouchedAC>
	| ReturnType<typeof setActivePageAC>
	| ReturnType<typeof setFinishAC>
	| ReturnType<typeof isNextLevelAC>
	| ReturnType<typeof isColoredAC>


export const initializedTC = (currentIndex: number, value: BirdType) => (dispatch: Dispatch<ActionsType>) => {
	dispatch(setIndexAC(currentIndex))
	dispatch(setRandomBirdAC(value))
}